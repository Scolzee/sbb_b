...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق فروزين
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@sbb_b.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق فروزين
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق فروزين
...
async def approve_p_m(event):
    ...
    if len(approved_users) > 0:
        for user in approved_users:
            APPROVED_PMs += f"• 𓆰 ََِِ𝖋𝖗𝖔𝖟𝖊𝖓 ََِِ𝘀ُُ𝗼ًًٍٍ𝗨ًًٍٍ𝗥ََِِ𝗰ََِِ𝗲 - 𝑨𝑷𝑷𝑹𝑶𝑽𝑬𝑫𝑺 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"
    else:
        APPROVED_PMs = "انت لم توافق على اي شخص بالاصل ️"
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية فروزين.txt",
