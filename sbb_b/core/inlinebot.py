...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧ𝖘𝖔𝖚𝖗𝖈𝖊 𝖋𝖗𝖔𝖟𝖊𝖓ⵧⵧⵧⵧ⍣**\n[ََِِ𝙁𝙍𝙊𝙕𝙀𝙉 ًًٍٍ𝗨ٍّّ𝘀ََِِ𝗲ًًٍٍ𝗥ًًٍٍ𝗕ُُ𝗼ٖٔ𝗧 🧸♥](https://t.me/Ch_frozen)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ببوت فروزين **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/Ch_frozen"),
                Button.url(
                    "المطور",
                    "t.me/wjj_u",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل بوت فروزين من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝖘𝖔𝖚𝖗𝖈𝖊 𝖋𝖗𝖔𝖟𝖊𝖓♥](https://t.me/Ch_frozen)",
