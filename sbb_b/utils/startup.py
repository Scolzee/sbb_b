...
LOGS = logging.getLogger("اعداد فروزين")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@x_x_x_g_o_o_o_o_o "))
    except BotMethodInvalidError:
        pass
    except ChannelsTooMuchError:
        LOGS.info("انضم بجروب السورس  اولا @x_x_x_g_o_o_o_o_o")
    except ChannelPrivateError:
        LOGS.critical(
            "تم حظرك من استخدام سورس فروزين  عليك الأعتذار الى مطور السورس @Scolze_WA"
        )
    try:
        await sbb_b(JoinChannelRequest("@Scolze_WA"))
    except BaseException:
        pass
    try:
        await sbb_b(JoinChannelRequest("@Ch_frozen"))
    except BaseException:
        pass
    try:
        await sbb_b(JoinChannelRequest("@M_O_D_Y_00"))
    except BaseException:
        pass
    try:
        await sbb_b(JoinChannelRequest("@x_x_x_g_o_o_o_o_o"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://t.me/xchhgbj/1565",
                caption="**تم تشغيل سورس فروزين بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/x_x_x_g_o_o_o_o_o"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/source_frozen.jpg")
        _, groupid = await create_supergroup(
            "جروب بوت فروزين", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
